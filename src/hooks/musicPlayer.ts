...
export function usePlayer(musicPlayer: any) {
  ...
  // 上一首或者下一首
  const goPrevOrNextMusic = useThrottleFn(flag => {
    switch (flag) {
      case 'prev':
  ...
  async function goPlayMusic() {
    ...
    const music = musicPlayer.playQueue[musicPlayer.currentMusicIndex]
    const musicArrayBuffer = await fetch(music.musicUrl).then(r => r.arrayBuffer())
  ...
  async function downloadMusic(music) {
    const musicUrl = await musicPlayer.getMusicUrl(music)
