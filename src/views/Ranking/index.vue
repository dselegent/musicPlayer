    ...
    <el-skeleton :rows="14" animated :loading="loading" :throttle="10">
      <template #default>
        <el-scrollbar height="440px" ref="scrollbarRef">
              ...
              <aside class="imgWrapper">
                <el-image :src="official?.coverImgUrl">
                  <template #placeholder>
                    <i i="ant-design-picture-filled" style="font-size: 104px; color: var(--el-color-info-light-3)"></i>
                  </template>
                </el-image>
          ...
          <h4>全球榜</h4>
          <!-- 全球榜列表 -->
          <SongSheetList :songSheetList="globalList" />
        </el-scrollbar>
      </template>
    </el-skeleton>
  </div>
...
<script setup lang="ts">
  ...
  onMounted(() => {
    ...
    2. 天数差大于等于1
  */
    let { officialListStorage, globalListStorage, reqTime } = JSON.parse(localStorage.getItem('rankList') || '{}')
