<template>
  <div class=".singerAlbum">
    ...
    <el-card class="albumCard">
      ...
      <div class="albumMain">
        ...
        <el-skeleton :rows="8" animated :loading="albumLoading" :throttle="10">
          <template #default>
            <el-table :data="singerInformation.handleArtistTopSong(currentPage)" stripe :show-header="false">
              <el-table-column width="340">
                <template #default="{ row }">
                  <div style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap">
                    {{ row.name }}
                    <span style="color: var(--el-text-color-secondary)" v-if="row?.alia[0]">
                      {{ `(${row?.alia[0]})` }}
                    </span>
                  </div>
                </template>
              ...
              <el-table-column align="right" width="150">
                <template #default="{ row }">
                  ...
                  <el-button link @click="downloadMusic(row)">
                    <i i="ant-design-download-outlined"></i>
                  </el-button>
                </template>
              </el-table-column>
              <template #empty>
    ...
    <!-- 专辑卡片 -->
    <el-card class="albumCard" v-for="(artistAlbum, index) in artistAlbumList" :key="artistAlbum.album?.id">
      <!-- 头部 -->
      <template #header>
        <div class="albumHeader">
          ...
          <el-image :src="artistAlbum.album?.picUrl" loading="lazy">
            <template #placeholder>
              <i
                ...
                style="margin-right: 12px; font-size: 100px; color: var(--el-color-info-light-3)"
              ></i>
            </template>
          </el-image>
        </div>
            ...
            <el-table :data="artistAlbum.songs.slice(0, 10)" stripe :show-header="false">
              <el-table-column width="340">
                <template #default="{ row }">
                  <div style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap">
                    {{ row.name }}
                    <span style="color: var(--el-text-color-secondary)" v-if="row.alia.length">
                      {{ `（${row.alia[0]}）` || '' }}
                    </span>
                  </div>
                </template>
...
<script setup lang="ts">
  ...
  onMounted(async () => {
    ...
    await singerInformation.getArtistTopSong(route.query?.id as any)
    albumLoading.value = false
  })
