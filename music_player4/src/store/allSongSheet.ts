...
const useAllSongSheetStore = defineStore('allSongSheet', {
  ...
  actions: {
    ...
    async getPlaylistCatlist() {
      let ret = await songSheet.reqPlaylistCatlist();
    ...
    async getPlaylistHot() {
      let ret = await songSheet.reqPlaylistHot();
      if (ret?.code !== 200) return ElMessage.error('获取热门歌单分类失败');
      this.playlistHot = [{ id: '全部歌单', name: '全部歌单' }, ...ret.tags];
    },
    // 根据关键字获取网友推荐歌单
    async getTopPlaylist(offset, cat, flag, limit = 15) {
      let ret = await songSheet.reqTopPlaylist(offset, limit, cat);
