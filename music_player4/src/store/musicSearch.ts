...
const useMusicSearchStore = defineStore('musicSearch', {
  ...
  actions: {
    ...
    async getsearchHotDetail() {
      let ret = await search.reqSearchHotDetail();
    ...
    async getMusicSearch({ keywords, type = 1, limit = 10, offset = 0 }) {
      let ret = await search.reqMusicSearch({
        keywords,
        type,
        limit,
        offset,
      ...
      switch (type) {
        ...
        case 1:
          ...
          // 如果有重复的就删除之前的，新添加一条
          let historyIndex = this.searchHistoryList.findIndex(item => item === keywords);
    ...
    async getSearchSuggestions(keywords) {
      let ret = await search.reqSearchSuggestions(keywords);
