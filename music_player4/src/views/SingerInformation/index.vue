    ...
    <el-scrollbar height="410px" ref="scrollbarRef">
          ...
          <el-image :src="artist.picUrl">
            <template #placeholder>
              <i
                ...
                style="font-size: 124px; color: var(--el-color-info-light-3)"
              />
            </template>
          </el-image>
        </div>
      ...
      <el-tabs v-model="activeName" @tab-click="handleClick" class="tabs">
        ...
        <el-tab-pane label="歌手详情" name="SingerDetails">
          <SingerDetails ref="SingerDetailsRef"></SingerDetails>
        </el-tab-pane>
        <el-tab-pane label="相似歌手" name="SimilarSinger">
          <SimilarSinger ref="SimilarSingerRef"></SimilarSinger>
        </el-tab-pane>
      </el-tabs>
    </el-scrollbar>
  </div>
...
const handleClick = (tab: TabsPaneContext) => {
  switch (tab.props.name) {
    ...
    case 'SingerDetails':
      // 发请求
      if (singerInformation.artistDesc.introduction.length === 0)
        SingerDetailsRef?.getArtistDesc(route.query?.id);
      break;
    case 'SimilarSinger':
      if (singerInformation.similarArtistList.length === 0)
        SimilarSingerRef?.getSimilarArtist(route.query?.id);
      break;
  }
};
